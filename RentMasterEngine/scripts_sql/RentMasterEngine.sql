...
CREATE PROCEDURE rentmasterengine.sp_pagar_alquiler_por_id_alquiler_y_monto( 
	...
	BEGIN
		...
		DECLARE deuda_alquiler DECIMAl(10,2); # VARIABLE PARA SABER EL MONTO DE X EQUIPO
		...
		IF existe_alquiler <> 0 THEN
			...
			SELECT a.id_equipo, a.id_cliente, a.total_pagar 
			INTO var_id_equipo, var_id_cliente ,deuda_alquiler
			FROM rentmasterengine.alquileres AS a
			WHERE id_alquiler = i_id_alquiler;
			
			
			IF i_monto_pagar > deuda_alquiler THEN 
			
				SET o_respuesta = CONCAT('SU DEUDA ES DE: ', deuda_alquiler, ', NO PUEDE PAGAR UNA CANTIDAD SUPERIOR A SU DEUDA');
					
				ROLLBACK ;
				
			ELSEIF i_monto_pagar = deuda_alquiler THEN
				...
				# ACTUALIZAMOS LA DEUDA DEL CLIENTE
			...
			ELSEIF i_monto_pagar < deuda_alquiler THEN
				...
				SET o_respuesta =  CONCAT('SU DEUDA ES DE: ', deuda_alquiler, ', SU PAGO ES MENOR A LA DEUDA ');
		...
		ELSE
			...
			SET o_respuesta =  'ESTE ALQUILER NO EXISTE';
...
CREATE TRIGGER rentmasterengine.tg_registrar_alquiler
	...
	BEGIN
		...
		IF estado_equipo = 'DISPONIBLE' THEN
			...
			UPDATE rentmasterengine.clientes 
			SET saldo_deudor = (saldo_deudor + NEW.total_pagar)
